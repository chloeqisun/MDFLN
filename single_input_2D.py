...
for train_index, test_index in kf.split(simple[:half_simple]):
    ...
    # 训练模型
    print('第', k + 1, '折：')
    history = model.fit(partial_x_train_tf,
                        partial_y_train,
                        epochs=100,
                        batch_size=32,
    ...
    test_loss[k], test_acc[k] = model.evaluate(x_test_tf, y_test)
    result_base = model.predict(x_test_tf)
    ...
    history = conv_base.fit(x_train_tf,
                            y_train,
                            epochs=30,
                            batch_size=32)
    # 测试模型
    test_loss[k], test_acc[k] = conv_base.evaluate(x_test_tf, y_test)
    result = conv_base.predict(x_test_tf)
...
# 基网络
np.save(simplePath + '\patient' + patients[0] + '\\tensecond\\base\ypred.npy', y_pred_base)
np.save(simplePath + '\patient' + patients[0] + '\\tensecond\\base\ytrue.npy', y_true_base)

# 微调模型
np.save(simplePath + '\patient' + patients[0] + '\\tensecond\\finetuning\ypred.npy', y_pred)
np.save(simplePath + '\patient' + patients[0] + '\\tensecond\\finetuning\ytrue.npy', y_true)
a = 0
